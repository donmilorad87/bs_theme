/**
 * Page Configuration Panel — Block Editor Plugin (vanilla JS).
 *
 * Adds a "Page Configuration" panel to the Document sidebar
 * with sidebar On/Off toggles, auto-detected override status,
 * and a Full Width toggle when no sidebars are active.
 *
 * @package BS_Custom
 */
(function (wp) {
	...
	var META_LEFT       = 'bs_sidebar_left';
	var META_RIGHT      = 'bs_sidebar_right';
	var META_FULL_WIDTH = 'bs_page_full_width';
	...
	function SidebarSettingsPanel() {
		...
		var fullWidthValue = meta[META_FULL_WIDTH]  || 'on';
		...
		var bothSidebarsOff = leftValue === 'off' && rightValue === 'off';
		...
		function onChangeFullWidth(checked) {
			var update = {};
			update[META_FULL_WIDTH] = checked ? 'on' : 'off';
			editPost.editPost({ meta: update });
		}
		...
		if (bothSidebarsOff) {
			panelChildren.push(
				el('div', {
					...
					el(ToggleControl, {
						label:    __('Full Width', 'ct-custom'),
						help:     __('When off, content is constrained to the container width.', 'ct-custom'),
						checked:  fullWidthValue === 'on',
						onChange: onChangeFullWidth,
					})
				)
			);
		}
		...
		return el(
			PluginDocumentSettingPanel,
			{
				...
				title: __('Page Configuration', 'ct-custom'),
				icon:  'columns',
			},
			panelChildren
		);
