 ...
 *
 * Scans the editor DOM (inside the editor iframe) for bs_translate()
...
(function (wp) {
	...
	/** Regex matching complete bs_translate() calls. */
	var PATTERN_RE = /bs_translate\(\s*['"]([A-Z][A-Z0-9_]*)['"]\s*(?:,\s*(?:\[(.*?)\]|\{(.*?)\})\s*(?:,\s*(?:['"]([a-z]+)['"]|(\d+))\s*)?)?\)/g;
	 ...
	 *
	 * @param {string}   patternText  Raw bs_translate(...) string.
	...
	/**
	 * Remove a bs_translate() pattern from a block's attributes.
	 *
	...
	/**
	 * Scan the editor DOM for bs_translate() patterns and
	...
	/**
	 * Replace a text node's bs_translate patterns with chip elements.
	 *
