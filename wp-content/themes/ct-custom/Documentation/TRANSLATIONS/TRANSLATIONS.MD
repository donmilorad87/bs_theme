# BS Custom — Multilanguage Translation System (Technical Reference)
...
The BS Custom theme includes a custom-built multilanguage system that supports:
...
- **Frontend resolution** of `bs_translate()` patterns in templates, Customizer values, block editor content, and REST API responses
...
| `TranslationService.php` | `TranslationService` | Static utility resolving `bs_translate()` patterns in customizer and block content. Three-phase block resolution (collect, translate deduplicated, replace). |
...
| `src/Customizer/Controls/TranslationControl.php` | `TranslationControl` | Custom WP Customizer control (`bs_translation` type) with "Pick Key" button and searchable dropdown. |
...
| `src/RestApi/Endpoints/ResolveTranslation.php` | `ResolveTranslation` | `POST /wp-json/ct-custom/v1/resolve-translation` — Resolves `bs_translate()` patterns in text. Max 2000 chars. |
...
PHP arrow syntax:
```
bs_translate('ITEM_COUNT', ['count' => '5'], 'other')
```
...
| Meta Key | Purpose |
|----------|---------|
| `bs_language` | Two-letter language code assigned to the page |
| `bs_locale` | Optional locale code (e.g., `en_GB`) |
| `bs_translation_group` | UUID linking all translations of the same page |
...
2. Phase 1: Regex collects all `bs_translate()` patterns
...
```json
{
    ...
    "data": {
        "iso2": "en",
        "translations": { "SITE_NAME": "BS Custom", ... }
    }
}
```
...
| `Translator::MAX_PATTERN_MATCHES` | 200 | Max bs_translate() patterns per resolve |
