# CT Custom — Translation System (Developer Guide)

> How to use the multilanguage system in the CT Custom theme.
...
```
ct_translate('GREETING', ['name' => 'Alice'])
```
...
echo ct_translate('SITE_NAME');

// Via the MultilangService singleton
$service = CTCustom\Multilang\MultilangService::instance();
...
Translation-aware controls show a "Pick Key" button. Click it to search and select a translation key. The value is stored as `ct_translate('KEY')` and resolved on the frontend.

### In Widgets

Widget text inputs with translation support show a "Pick Key" button. Select a key and the input value becomes `ct_translate('KEY')`. A yellow preview shows the resolved text.
...
Body: { "text": "ct_translate('SITE_NAME')", "iso2": "en" }
```
...
Controls registered with `ct_translation` type show a translation key picker. Used for site description and footer copyright.
...
The `editor-translator.js` script enables live preview of `ct_translate()` patterns in the block editor. No special UI needed; just type the pattern.
...
{
    "text": "ct_translate('SITE_NAME')",
    "iso2": "en"
}
```

Resolves `ct_translate()` patterns in the text. The `iso2` parameter is optional (defaults to current language). Maximum text length: 2000 characters.

---

## Extending — Custom Plural Rules
...
|----------|------|-------------|
| `ct_language` | string | Two-letter ISO 639-1 language code (e.g., `en`, `sr`) |
| `ct_locale` | string | Optional locale code for region-specific content (e.g., `en_GB`) |
| `ct_translation_group` | string | UUID linking all translations of the same source page |
